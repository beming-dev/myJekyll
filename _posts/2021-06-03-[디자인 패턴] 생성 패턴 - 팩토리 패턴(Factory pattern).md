...
패턴 하나를 정리하는데에 생각보다 많은 시간이 걸렸다. 솔직히 처음에 추상 팩토리 사용이유도 딱히 이해가 안되고, 예제를 정리하면서도 헷갈려서 엄청 해맸다. 겨우 다 이해하고 글을 다 작성하고도 굳이 이렇게까지 작성해야하나..? 라는 생각이 들었다. 하지만 디자인 패턴은 협업에서 더 필요한것이기도 하고, 내가 객체지향 프로그래밍을 제대로 해본 경험이 없어서 그렇게 느낀것 같다. 그래도 미리 알아두면 나중에 필요할 때 잘 써먹을 수 있을 것이다.
