...
<img src="/assets/images/2021-03-26-라즈베리 파이(Raspberry pi) 서버로 사용하기_OS 설치 (Ubuntu)/1.png" alt="내 커뮤니티"/>
...
<img src="/assets/images/2021-03-26-라즈베리 파이(Raspberry pi) 서버로 사용하기_OS 설치 (Ubuntu)/3.png" alt="조립사진"/>
...
이제 sd카드를 제거하고 라즈베리 파이에 꽂아주면 된다. 그리고 ssh로 접속하면 된다. ssh로 접속하기 위해서는 라즈베리 파이의 ip주소를 알아야 한다.
...
이렇게 생각보다는 쉬운(?) OS설치가 끝났다. 나는 처음에 우분투 이미지가 sd카드에 잘못 구워져서 공유기 사이트에 라즈베리 파이가 뜨지 않았다. 멍청했던 나는 라즈베리 파이가 부팅이 이렇게 오래걸리나..? 싶어 10분이고 20분이고 기다렸지만 될리가 없었다. 라즈베리파이에서 LAN선을 20번정도 꼈다 뺐다 한 후에야 혹시 이미지가 잘못구워진걸까 싶어 다시 구워봤더니 바로 됐다ㅠㅠ 라즈베리 파이 부팅은 30초면 되는거였다. 이 과정에서 sd카드에 이미지를 다시 굽는 중에도 파티션을 다 날려버려서 복구하는 방법 찾는데 시간이 엄청 들었다.
